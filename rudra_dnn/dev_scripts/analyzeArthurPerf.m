%% perfLogFile: 
function analyzeArthurPerf(perfLogFile, p, e, epochNum)
    ...
    legend('load', 'bcast', 'deser\_weights', 'sel\_train\_data', 'pull\_w', 'train','s\_updates', 'push\_updates', 'report\_train\_err','test','report\_test\_err');
    %%%title('\fontsize{16}Where did time go ?')
    xlabel('\fontsize{16}Number of learners')
    ylabel('\fontsize{16}Training time one epoch')
end
