...
void BinarySampleMFReader::readLabelledSamples(size_t numSamples,
	...
	for (size_t i = 0; i < numSamples; ++i) {
	    //idx[i] = rand() % totalSampleNum + 1; // we have a contract that idx is between 1 and totalNumSample
	...
	for (size_t i = 0; i < numSamples; ++i) {
	    ...
	    std::cout<<"[idx X] "<<i<<" "<<myXIdx<<std::endl;
	    std::cout<<"[idx Y] "<<i<<" "<<myYIdx<<std::endl;
